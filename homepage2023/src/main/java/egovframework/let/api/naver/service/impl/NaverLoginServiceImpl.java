...
public class NaverLoginServiceImpl extends EgovAbstractServiceImpl implements NaverLoginService {
		...
		public String getAuthorizationUrl(HttpSession session, String domain, String port) {
			...
			String state = generateRandomString();
			 /*생성한 난수 값을 session 에 저장*/
			setSession(session,state);
			 /*scrive에서 제공하는 인증 url 생성기능을 이용하여 네이버아이디로로 인증 url 생성*/
		...
		public OAuth2AccessToken getAccessToken(HttpSession session, String code, String state, String domain,
			...
			if(StringUtils.pathEquals(sessionState, state)) {
				...
				//scribe에서 제공하는 accessToken 획득 기능으로 네이버아이디로 access token 획득
		...
		//access token을 이용한 네이버 사용자 프로필 api 호출 
