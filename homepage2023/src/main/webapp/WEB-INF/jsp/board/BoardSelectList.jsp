...
<div class="container">
	...
	<table class="list_table">
		<thead>
			...
			<td class="hits" scope="col">조회수</th>
			</tr>
		</thead>
		<tbody>
	<%--공지글--%>
	<c:forEach var="result" items="${noticeResultList}" varStatus="status">
		<tr class="notice">
			...
			<td class="writer" date-cell-header="작성자: "><c:out value="${result.frstRegisterId}"/></th>
			<td class="date" date-cell-header="작성일: "><fmt:formatDate value="${result.frstRegistPnttm}" pattern="yyyy-MM-dd"/></th>
			<td class="hits" date-cell-header="조회수: "><c:out value="${result.inqireCo}"/></th>
			</tr>
			</c:forEach>
	<%--일반글 --%>
