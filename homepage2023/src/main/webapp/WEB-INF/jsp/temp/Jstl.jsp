	...
	<c:choose>
		...
		<c:when test="${step eq '1'}">
			<li>
				...
				JSP 1번 : hello <% out.print("hello~"); %><br/>
				JSP 2번 : <% String a = "hello~"; %>
							<%=a%><br/>
				JSPL : <c:out value="hello~"/><br/>
			</li>
		</c:when>
		...
		<c:when test="${step eq '3'}">
			<li>
				<h3>c:set Tag : 일반변수를 생성해서 값을 할당</h3>
		...
		<c:when test="${step eq '5'}">
			<li>
				...
				<h4>기본문법 :<br/>
				 ...
				 &#60;c:otherwise&#62; 조건에 해당하지 않아서 나오는 내용 &#60;/c:otherwise&#62;      <br/>  
				  &#60;/c:choose&#62; 
				 </h4>
								
				<c:set var="test" value="0"/>
				<c:choose>
					<c:when test="${step eq '5'}">1입니다.</c:when>
		...
		<c:when test="${step eq '14'}">
			<li>
				<h3>fn:length() : 문자열의 내부의 문자 수 또는 컬렉션의 항목 수를 반환</h3>
		...
		<c:when test="${step eq '18'}">
			<li>
				<h3>fn:replace() : 모든 문자열을 다른 문자열의 시퀀스로 바꿈</h3>
		...
		<c:when test="${step eq '20'}">
			<li>
				<h3>fmt:formatDate(): 제공된 패턴 및 스타일을 사용하여 시간 및 날짜를 형식화함
				</h3>
